...
assert.equal(cfg.patch_level,'SMOOTH_FLOW_BLACK_HOLE_DYSON_1');
...
assert.equal(cfg.rendering.dyson_sphere,'CENTERED_SLOT_BACKDROP_PARTIAL_SWARM_LATTICE');
assert.equal(cfg.rendering.dyson_viewport_anchor_percent_x,35.5);
assert.equal(cfg.rendering.dyson_behind_shell,true);
assert.equal(cfg.rendering.dyson_external_assets,false);
assert.equal(cfg.rendering.dyson_network_requests,false);
assert.equal(cfg.rendering.dyson_reduced_motion_disables_rotation,true);
assert.equal(cfg.rendering.dyson_gameplay_authority,'NONE');
...
assert.match(source,/PATCH_LEVEL = 'SMOOTH_FLOW_BLACK_HOLE_DYSON_1'/);
...
assert.match(source,/ensureDysonSphere/);
assert.match(source,/helios-dyson-sphere/);
assert.match(source,/left:35\.5%/);
assert.match(source,/PARTIAL_SWARM_LATTICE/);
assert.match(source,/helios-dyson-ring-a/);
assert.match(source,/helios-dyson-ring-b/);
assert.match(source,/helios-dyson-breathe/);
assert.match(source,/dyson_gameplay_authority:'NONE'/);
...
assert.doesNotMatch(source,/querySelector\(['"]#game-panel['"]\)/);
...
assert.match(docs,/SMOOTH_FLOW_BLACK_HOLE_DYSON_1/);
...
assert.match(docs,/Dyson sphere behind the slot/i);
assert.match(docs,/partial Dyson swarm \/ lattice sphere/i);
