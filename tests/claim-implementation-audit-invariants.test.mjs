...
assert.equal(status.pilot_authority.version, '1.1.0');
assert.equal(status.pilot_authority.maturity, 'IMPLEMENTED_CORE_ARMED_DISABLED_PENDING_RECEIVING_ADDRESS_AND_RPC');
...
assert.equal(status.pilot_authority.dedicated_rpc_required, true);
assert.equal(status.pilot_authority.dedicated_rpc_configured, false);
...
assert.equal(architecture.pilot_authority.version, '1.1.0');
assert.equal(architecture.pilot_authority.maturity, 'IMPLEMENTED_CORE_ARMED_DISABLED_PENDING_RECEIVING_ADDRESS_AND_RPC');
...
assert.equal(architecture.pilot_authority.primary_payment.dedicated_rpc_required_before_enable, true);
...
assert.equal(byClaim.get('AUTOMATED_STANDARD_PILOT_AUTHORITY').claim_value, 'ARMED_DISABLED_PENDING_RECEIVING_ADDRESS_AND_RPC');
assert.match(byClaim.get('AUTOMATED_STANDARD_PILOT_AUTHORITY').external_gate, /Binance USDT\/Ethereum ERC20/i);
assert.match(byClaim.get('AUTOMATED_STANDARD_PILOT_AUTHORITY').external_gate, /dedicated Ethereum RPC/i);
...
assert.equal(pilotContract.version, '1.1.0');
assert.equal(pilotContract.status, 'ARMED_DISABLED_PENDING_RECEIVING_ADDRESS_AND_RPC');
...
assert.equal(pilotContract.payment.dedicated_rpc_required_before_enable, true);
assert.equal(pilotContract.security.wallet_private_key_required, false);
assert.equal(pilotPolicy.version, '1.1.0');
...
assert.equal(pilotPolicy.chain_observation.rpc_url, null);
