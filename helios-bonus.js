(() => {
  ...
  function injectStyles(){
    ...
    style.textContent=`
      ...
      @media(max-width:520px){.corona-stage{width:275px;height:275px}.corona-sun{width:96px;height:96px}.corona-pointer{height:111px}.corona-ray{width:60px;font-size:10px}}
    `;
  ...
  function buildUI(){
    ...
    overlay.innerHTML=`<section class="solar-corona-card" role="dialog" aria-modal="true" aria-labelledby="solar-corona-title">
      <div class="solar-corona-kicker">☀ HELIOS FEATURE</div>
      <h3 id="solar-corona-title" class="solar-corona-title">SOLAR CORONA BONUS</h3>
      <div id="solar-corona-sub" class="solar-corona-sub">3 SUNS IGNITE THE CORONA</div>
      <div class="corona-stage"><div class="corona-ring"></div><div id="corona-rays"></div><div id="corona-pointer" class="corona-pointer"></div><div class="corona-sun">☀</div></div>
      <div id="solar-corona-result" class="solar-corona-result">IGNITING…<small>DEMO BONUS FEATURE</small></div>
      <div class="solar-corona-ledger"><b>SOLAR BONUS BANK</b> · separate demo-only ledger · no real-money value · no compute or route effect.</div>
    </section>`;
    ...
    if(tools && !$('solar-bonus-chip')){
      ...
      const mini=tools.querySelector('.session-mini');
  ...
  function renderRays(){
    ...
    if(!host) return;
    host.innerHTML='';
    const values=policy.multipliers;
    values.forEach((m,i)=>{
  ...
  function countTriggerSymbols(){
    const symbol=String(policy.trigger_symbol||'☀');
    return qa('#reels .cell').filter(c=>c.textContent.trim()===symbol).length;
  }

  function eligible(){
    return Boolean(policy.enabled && policy.eligible_game_modes.includes(currentMode()));
  }

  async function triggerBonus(symbolCount){
    ...
    const reward=Math.round(currentBet()*multiplier*100)/100;
    ...
    bonusBank=Math.round((bonusBank+reward)*100)/100;
    bonusCount++;
    ...
    window.dispatchEvent(new CustomEvent('helios:solar-corona',{detail:{
  ...
  function inspectSettledSpin(){
    ...
    if(count>=policy.trigger_count) triggerBonus(count);
  }
