(() => {
  ...
  const DEFAULT_POLICY = {enabled:true,default_on:false,master_gain:0.30,scheduler_ms:25,lookahead_seconds:0.14,profiles:DEFAULT_PROFILES,route_profiles:DEFAULT_ROUTE_PROFILES};
  const PROGRESSION=[0,4,5,3],BASE_KICKS=[0,4,8,12],BASS_STEPS=[0,3,6,8,11,14],STAR_STEPS=[5,13];
  ...
  const state={
    ...
    mode:document.body.dataset.gameMode||'helios',route:$('selected-route')?.textContent?.trim()||'MARKET',
    computeActive:false,eventEnergy:0,boostBars:0,solarBars:0,luckyBars:0,bonusBuyBars:0,fillSteps:0,cascadeTier:0,
    bonusSessionActive:false,bonusSessionSpin:0,sessionSeed:0,attached:false
  };
  ...
  async function loadPolicy(){
    try{
      const r=await fetch('./config/helios.public.json',{cache:'no-store'});if(!r.ok)return;const cfg=await r.json(),p=cfg?.procedural_audio||{};
      const profiles={...DEFAULT_PROFILES};
      if(p.profiles&&typeof p.profiles==='object')for(const [key,raw] of Object.entries(p.profiles)){if(!DEFAULT_PROFILES[key]||!raw)continue;const base=DEFAULT_PROFILES[key],scale=Array.isArray(raw.scale)&&raw.scale.length?raw.scale.map(Number).filter(Number.isFinite):base.scale,motif=Array.isArray(raw.motif)&&raw.motif.length?raw.motif.map(Number).filter(Number.isFinite):base.motif;profiles[key]={...base,...raw,root_midi:clamp(raw.root_midi??base.root_midi,24,84),bpm:clamp(raw.bpm??base.bpm,40,110),scale,motif};}
      const route_profiles={...DEFAULT_ROUTE_PROFILES};
      if(p.route_profiles&&typeof p.route_profiles==='object')for(const [key,raw] of Object.entries(p.route_profiles)){const upper=String(key).toUpperCase();if(!DEFAULT_ROUTE_PROFILES[upper]||!raw)continue;const base=DEFAULT_ROUTE_PROFILES[upper];route_profiles[upper]={...base,...raw,bpm_delta:clamp(raw.bpm_delta??base.bpm_delta,-16,20),degree_shift:clamp(raw.degree_shift??base.degree_shift,-7,7),pulse_density:clamp(raw.pulse_density??base.pulse_density,.1,1),bass_density:clamp(raw.bass_density??base.bass_density,.1,1),arp_density:clamp(raw.arp_density??base.arp_density,.1,1),star_density:clamp(raw.star_density??base.star_density,.05,1),filter_bias:clamp(raw.filter_bias??base.filter_bias,-1200,1600),drone_gain:clamp(raw.drone_gain??base.drone_gain,.004,.05)};}
  ...
  function ensureGraph(){if(state.ctx){if(state.ctx.state==='suspended')state.ctx.resume().catch(()=>{});return state.ctx;}const Ctx=window.AudioContext||window.webkitAudioContext;if(!Ctx)return null;const ctx=new Ctx(),master=ctx.createGain(),filter=ctx.createBiquadFilter(),delay=ctx.createDelay(1.5),feedback=ctx.createGain(),reverb=ctx.createConvolver(),compressor=ctx.createDynamicsCompressor();master.gain.value=.0001;filter.type='lowpass';filter.frequency.value=3600;filter.Q.value=.55;delay.delayTime.value=.285;feedback.gain.value=.22;reverb.buffer=impulse(ctx);compressor.threshold.value=-18;compressor.knee.value=20;compressor.ratio.value=3.2;compressor.attack.value=.008;compressor.release.value=.38;filter.connect(master);filter.connect(delay);delay.connect(feedback);feedback.connect(delay);delay.connect(master);filter.connect(reverb);reverb.connect(master);master.connect(compressor);compressor.connect(ctx.destination);Object.assign(state,{ctx,master,filter,delay,feedback,reverb,compressor,noiseBuffer:makeNoiseBuffer(ctx)});return ctx;}
  ...
  function eventTempoDelta(){return (state.cascadeTier>=4?18:state.cascadeTier===3?12:state.cascadeTier===2?7:state.cascadeTier===1?3:0)+(state.solarBars>0?18:0)+(state.luckyBars>0?12:0)+(state.bonusBuyBars>0?10:0)+(state.bonusSessionActive?12:0)+(state.fillSteps>0?2:0);}
  function effectiveBpm(){return clamp(Number(profile().bpm||66)+Number(routeProfile().bpm_delta||0)+eventTempoDelta(),48,132);}
  function stepSeconds(){return (60/effectiveBpm())/4;}

  function scheduleTransportStep(when){
    const p=profile(),rp=routeProfile(),step=state.step,root=chordRoot(),boost=state.boostBars>0||state.solarBars>0||state.luckyBars>0||state.bonusBuyBars>0||state.bonusSessionActive||state.fillSteps>0,energy=clamp(state.eventEnergy,0,1),variation=unitRand(1234);
    if(state.filter)state.filter.frequency.setTargetAtTime(clamp(3400+Number(rp.filter_bias||0)+energy*1400+(state.solarBars>0?1200:0)+(state.bonusSessionActive?650:0),900,7600),when,.05);
    if(step===0){const chord=[root,root+2,root+4].map(d=>midiToHz(modeMidi(d,-1)));pad(chord,when,stepSeconds()*15.2,.026+(state.solarBars>0?.014:0)+(state.luckyBars>0?.006:0)+(state.bonusSessionActive?.009:0));}
    const kickBase=BASE_KICKS.includes(step),routeExtra=(rp.pulse_density>.8&&[2,6,10,14].includes(step)&&variation<rp.pulse_density-.55),bonusExtra=state.bonusSessionActive&&[2,6,10,14].includes(step)&&unitRand(1300+step)<.62;if(kickBase||routeExtra||bonusExtra)kick(when,.72+(state.mode==='gridjack'?.10:0)+(boost?.10:0)+(state.route==='TREASURY'?.10:0)+(state.bonusSessionActive?.08:0));
    if(step%2===0&&unitRand(2000+step)<rp.pulse_density)noiseHit(when,.011+(boost?.005:0)+(state.bonusSessionActive?.003:0),5900,.035,step%4===0?-.25:.25);
    if(boost&&step%2===1&&unitRand(3000+step)<Math.min(1,rp.pulse_density+.18))noiseHit(when,.0075+(state.bonusSessionActive?.002:0),7000,.024,step%4===1?.45:-.45);
    if(BASS_STEPS.includes(step)&&unitRand(4000+step)<Math.min(1,rp.bass_density+(state.bonusSessionActive?.10:0))){const bassDegrees=[0,0,2,0,4,2],d=root+bassDegrees[BASS_STEPS.indexOf(step)];voice(midiToHz(modeMidi(d,-2)),when,.24,.068+(energy*.014)+(state.route==='TREASURY'?.010:0)+(state.bonusSessionActive?.008:0),'triangle',0,720,step<8?-.18:.18);}
    const motif=p.motif?.length?p.motif:[0,2,4,1,3,5,2,4],arpActive=(step%2===1&&unitRand(5000+step)<Math.min(1,rp.arp_density+(state.bonusSessionActive?.16:0)))||boost;
    if(arpActive){const mutate=unitRand(5100+step)>.72?(unitRand(5200+step)>.5?1:-1):0,degree=root+motif[(step+state.bar+state.bonusSessionSpin)%motif.length]+mutate,octave=state.cascadeTier>=3?2:state.cascadeTier>=1?1:(state.bonusSessionActive&&step>=8?2:step>=8?1:0);voice(midiToHz(modeMidi(degree,octave)),when,.13,.026+(energy*.013)+(state.solarBars>0?.010:0)+(state.luckyBars>0?.005:0)+(state.bonusSessionActive?.006:0),p.wave||'sine',0,4200+(energy*1800),(step%4-1.5)/2.2);}
    const starChance=rp.star_density+(state.solarBars>0?.28:0)+(state.luckyBars>0?.20:0)+(state.bonusSessionActive?.18:0);if(STAR_STEPS.includes(step)||unitRand(6000+step)<starChance*.08){bell(midiToHz(modeMidi(root+6+(unitRand(6100+step)>.5?0:2),2)),when,.014+(state.solarBars>0?.012:0)+(state.luckyBars>0?.009:0)+(state.bonusSessionActive?.005:0),step<8?.55:-.55);}
  ...
  function onBonusSessionStart(){state.bonusSessionActive=true;state.bonusSessionSpin=0;state.bonusBuyBars=Math.max(state.bonusBuyBars,4);state.boostBars=Math.max(state.boostBars,4);state.fillSteps=Math.max(state.fillSteps,12);state.eventEnergy=.95;eventChord([0,2,4,6],{octave:1,gain:.050,spread:.045});updateStatus();}
  function onBonusSpin(e){state.bonusSessionSpin=Number(e.detail?.spin_number||state.bonusSessionSpin+1);const retrigger=Number(e.detail?.retrigger_spins_added||0)>0,peak=Number(e.detail?.peak_multiplier||1),won=Number(e.detail?.spin_win||0)>0;state.boostBars=Math.max(state.boostBars,retrigger?3:won?2:1);state.fillSteps=Math.max(state.fillSteps,retrigger?12:peak>=16?10:6);state.eventEnergy=Math.min(1,state.eventEnergy+(retrigger?.42:won?.24:.10));if(retrigger)eventChord([0,3,6,8],{octave:1,gain:.060,spread:.035});else if(peak>=16)eventChord([0,2,5,7],{octave:1,gain:.052,spread:.04});else eventChord(won?[0,4,2]:[0,1],{octave:won?1:0,gain:won?.038:.024,spread:.05});updateStatus();}
  function onBonusSessionComplete(){state.bonusSessionActive=false;state.bonusBuyBars=Math.max(state.bonusBuyBars,2);state.boostBars=Math.max(state.boostBars,3);state.fillSteps=Math.max(state.fillSteps,10);state.eventEnergy=.9;eventChord([0,4,6,9],{octave:1,gain:.058,spread:.055});updateStatus();}
  function onBonusBuyComplete(){if(!state.bonusSessionActive){state.bonusBuyBars=Math.max(state.bonusBuyBars,2);state.boostBars=Math.max(state.boostBars,2);eventChord([4,6,2],{octave:1,gain:.046,spread:.05});}}
  ...
  function updateStatus(){const chip=$('cosmic-music-status');if(chip)chip.textContent=state.enabled?`${profile().name} · ${state.route} · ${Math.round(effectiveBpm())} BPM${state.bonusSessionActive?' · BONUS':''}`:'SILENT';const btn=$('sound-toggle');if(btn){btn.textContent=state.enabled?'♫ COSMIC AUDIO ON':'♫ COSMIC AUDIO OFF';btn.classList.toggle('active',state.enabled);}}
  ...
  function bindGameEvents(){
    window.addEventListener('helios:cascade',onCascade);window.addEventListener('helios:spin-complete',onSpinComplete);window.addEventListener('helios:solar-corona',onSolarCorona);window.addEventListener('helios:spin-energy-earned',onSpinEnergyEarned);window.addEventListener('helios:lucky-contribution',onLuckyContribution);window.addEventListener('helios:bonus-buy-start',onBonusBuyStart);window.addEventListener('helios:bonus-session-start',onBonusSessionStart);window.addEventListener('helios:bonus-spin',onBonusSpin);window.addEventListener('helios:bonus-session-complete',onBonusSessionComplete);window.addEventListener('helios:bonus-buy-complete',onBonusBuyComplete);$('spin')?.addEventListener('click',spinIgnition,{capture:true});
  }
  ...
  async function init(){state.sessionSeed=secureUint();await loadPolicy();bindGameEvents();observeEnvironment();bindLifecycle();let attempts=0;const attach=()=>{buildStatusChip();if(bindSoundToggle()&&buildStatusChip())return;if(++attempts<80)setTimeout(attach,75);};attach();}
