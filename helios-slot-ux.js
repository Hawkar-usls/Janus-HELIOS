(() => {
  ...
  const MODES = {
    helios:{name:'HELIOS',subtitle:'Universal Core',lines:3,payoutScale:1,core:'☀',symbols:[['☀',1.60],['⬡',1.30],['◈',1.10],['⚙',.90],['✦',.72],['∆',.52],['◇',.36]],feature:'3+ ☀ anywhere on a settled non-bonus grid can ignite the natural Solar Corona wheel. BUY BONUS opens a tier chooser for a separate Solar Free Spins session.'},
  ...
  const BONUS_TIERS = [
    {name:'STANDARD',multiplier:50,spins:10,retrigger:2,max:16},
    {name:'RADIANT',multiplier:100,spins:12,retrigger:2,max:20},
    {name:'SOLAR FLARE',multiplier:175,spins:15,retrigger:3,max:24}
  ];
  ...
  function guideHtml(){
    ...
    const tiers=BONUS_TIERS.map(t=>`<div class="slot-guide-tier"><b>${t.name}</b><strong>${t.multiplier}× BET · ${round2(bet*t.multiplier).toFixed(2)}</strong><span>${t.spins} free spins · 3+ ☀ → +${t.retrigger} · max ${t.max}</span></div>`).join('');
    return `<div class="slot-guide-head"><div><h3>ⓘ HELIOS GAME GUIDE · ${mode.name}</h3><p>Transparent demo rules for the currently selected profile. Values below recalculate from the current BET.</p></div><button id="slot-guide-close" class="slot-guide-close" type="button">✕</button></div>
      ...
      <section class="slot-guide-section"><h4>FEATURES</h4><div class="slot-guide-feature"><strong>${mode.name}</strong> — ${mode.feature}<br><br><strong>CASCADES</strong> — every paid line removes its winning symbols, the remaining symbols collapse, new symbols refill from above, and another paid cascade advances the ladder x1 → x4 → x16 → x64.<br><br><strong>HELIOS BONUS BUY</strong> — choose one of three disclosed demo tiers. Higher tiers buy more free-spin opportunities and a larger natural-sun retrigger budget; they do not guarantee a win. The exact price is reviewed and explicitly confirmed before anything is deducted.<div class="slot-guide-tier-grid">${tiers}</div><br><strong>WIN FOCUS</strong> — non-winning cells temporarily dim while paid hit cells and the trace remain emphasized.</div></section>
